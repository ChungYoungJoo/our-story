...
**2026-09-23 변경**: 아이들이 즐거워해서 쓰기 흐름을 손봤다.
① 내 글도 카드로 보이게(전에는 계속 편집 상태였다) ② 하루 5개까지.
DB 는 `supabase/migration-2026-09-23-multi-stories.sql` 로 유일 인덱스를 지우고 트리거를 걸었다.
화면 동작은 기기 저장 모드로 바꿔 5개 쓰기·한도 잠김·고치기·지우기·남의 글엔 버튼 없음까지 확인했다.
**DB 트리거는 실행해 보지 못했다**(Node 도 psql 도 없다) — 5개를 넘겨 저장될 때 확인할 것.
...
- **한 사람이 하루에 5개까지** 쓸 수 있다 (2026-09-23 에 '하루 하나'에서 바꿨다).
  개수는 `util.js` 의 `MAX_STORIES_PER_DAY` 와 DB 트리거 `stories_per_day_limit()` **두 곳**에 있다.
  바꾸려면 둘 다 바꿀 것. 화면은 한도에 닿으면 쓰는 칸을 잠그고, store 와 DB 가 한 번 더 막는다
- **쓰는 칸과 올라간 글은 분리한다.** 저장하면 내 글도 남의 글과 같은 카드로 보이고,
  '고치기' 를 눌렀을 때만 그 카드가 편집 상태(`state.editingStory`)가 된다.
  고치기·지우기 버튼은 **자기 글에만** 붙인다
